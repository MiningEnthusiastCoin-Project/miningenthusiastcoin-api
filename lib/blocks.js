...
BlockController.prototype.getBlockByHash = function (hash, next) {
    ...
    if (blockCached) {
        ...
        blockCached.confirmations = self.node.services.miningenthusiastcoind.height - blockCached.height + 1;
        ...
        return async.waterfall([function (callback) {
            ...
            return self.node.services.miningenthusiastcoind.getBlockHeader(hash, function (err, info) {
...
BlockController.prototype.blockIndex = function (req, res) {
    ...
    this.node.services.miningenthusiastcoind.getBlockHeader(parseInt(height), function (err, info) {
        if (err) {
            return self.common.handleErrors(err, res);
        }
        res.jsonp({
...
BlockController.prototype._getBlockSummary = function (hash, moreTimestamp, next) {
    ...
    if (summaryCache) {
        ...
        return async.waterfall([function (callback) {
            return self.node.services.miningenthusiastcoind.client.getBlock(hash, function (err, response) {
            ...
            var confirmations = self.node.services.miningenthusiastcoind.height - block.height + 1;
...
BlockController.prototype.list = function (req, res) {
    ...
    return self.node.services.miningenthusiastcoind.getBlockHashesByTimestamp(lte, gte, function (err, hashes) {
