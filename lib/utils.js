...
UtilsController.prototype.estimateFee = function(req, res) {
  ...
  async.map(nbBlocks, function(n, next) {
    ...
    self.node.services.miningenthusiastcoind.estimateFee(num, function(err, fee) {
      if (err) {
        return next(err);
      }
      next(null, [num, fee]);
...
UtilsController.prototype.minEstimateFee = function(req, res) {
    ...
    return self.node.services.miningenthusiastcoind.estimateFee(nBlocks, function(err, fee) {
